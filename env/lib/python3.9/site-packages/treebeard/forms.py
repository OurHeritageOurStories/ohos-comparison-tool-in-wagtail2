...
class MoveNodeForm(forms.ModelForm):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.declared_fields["_ref_node_id"].coerce = (
            pkFormField.to_python if pkFormField else int
        )
    ...
    def is_loop_safe(for_node, possible_parent):
        if for_node is not None:
            return not (possible_parent == for_node) or (
    ...
    def add_subtree(cls, for_node, node, options):
        ...
        if cls.is_loop_safe(for_node, node):
            for item, _ in node.get_annotated_list(node):
                options.append(
                    (item.pk, mark_safe(cls.mk_indent(item.get_depth()) + escape(item)))
                )
