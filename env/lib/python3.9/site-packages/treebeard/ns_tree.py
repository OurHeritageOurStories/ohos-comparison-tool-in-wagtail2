...
class NS_Node(Node):
    ...
    def move(self, target, pos=None):
        ...
        if self == target and (
            (pos == "left")
            or (
                pos in ("right", "last-sibling") and target == target.get_last_sibling()
            )
    ...
    def load_bulk(cls, bulk_data, parent=None, keep_ids=False):
        ...
        while stack:
            ...
            if "children" in node_struct:
                ...
                # the new nodes
                stack.extend(
                    [(node_obj.pk, node) for node in node_struct["children"][::-1]]
                )
        return added
    ...
    def get_ancestors(self):
        ...
        return get_result_class(self.__class__).objects.filter(
            tree_id=self.tree_id, lft__lt=self.lft, rgt__gt=self.rgt
        )
    ...
    def is_descendant_of(self, node):
        ...
        """
        return (
            self.tree_id == node.tree_id and self.lft > node.lft and self.rgt < node.rgt
        )
