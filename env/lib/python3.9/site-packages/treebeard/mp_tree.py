...
from treebeard.exceptions import InvalidMoveToDescendant, PathOverflow, NodeAlreadySaved
...
class MP_ComplexAddMoveHandler(MP_AddHandler):
    ...
    def get_sql_update_numchild(self, path, incdec="inc"):
        ...
        sql = "UPDATE %s SET numchild=numchild%s1" " WHERE path=%%s" % (
    ...
    def reorder_nodes_before_add_or_move(
        ...
        if (pos == "last-sibling") or (
            ...
            if movebranch:
                self.stmts.append(self.get_sql_newpath_in_branches(oldpath, newpath))
        else:
            # do the UPDATE dance
            ...
            if newpos is None:
                ...
                newpos = {"first-sibling": 1, "left": basenum, "right": basenum + 1}[
                    pos
                ]
            ...
            if movebranch and len(oldpath) == len(newpath):
                parentoldpath = self.node_cls._get_basepath(
                    oldpath, int(len(oldpath) / self.node_cls.steplen) - 1
                )
                ...
                if parentoldpath == parentnewpath and siblings and newpath < oldpath:
                    ...
                    self.stmts.append(
                        self.get_sql_newpath_in_branches(oldpath, tempnewpath)
                    )
            ...
            if movebranch:
                ...
                if tempnewpath:
                    self.stmts.append(
                        self.get_sql_newpath_in_branches(tempnewpath, newpath)
                    )
                else:
                    self.stmts.append(
    ...
    def get_sql_newpath_in_branches(self, oldpath, newpath):
        ...
        if vendor == "mysql":
            ...
            sqlpath = sql_concat(
                "%s", sql_substr("path", "%s", vendor=vendor), vendor=vendor
            )
        ...
        if len(oldpath) != len(newpath) and vendor != "mysql":
            ...
            sql2.append(
                ("depth=" + sql_length("%s", vendor=vendor) + "/%%s") % (sqlpath,)
            )
...
class MP_AddChildHandler(MP_AddHandler):
    ...
    def process(self):
        ...
        get_result_class(self.node_cls).objects.filter(path=self.node.path).update(
...
class MP_AddSiblingHandler(MP_ComplexAddMoveHandler):
    ...
    def process(self):
        ...
        if parentpath:
            self.stmts.append(self.get_sql_update_numchild(parentpath, "inc"))
...
class MP_MoveHandler(MP_ComplexAddMoveHandler):
    ...
    def process(self):
        ...
        if oldpath == self.target.path and (
            (self.pos == "left")
            or (
                self.pos in ("right", "last-sibling")
                and self.target.path == self.target.get_last_sibling().path
            )
            or (
                self.pos == "first-sibling"
                and self.target.path == self.target.get_first_sibling().path
            )
        ):
    ...
    def sanity_updates_after_move(self, oldpath, newpath):
        ...
        """
        if self.node_cls.get_database_vendor("write") == "mysql" and len(
            oldpath
        ) != len(newpath):
        ...
        if (
            (not oldparentpath and newparentpath)
            or (oldparentpath and not newparentpath)
            or (oldparentpath != newparentpath)
        ):
            # node changed parent, updating count
            if oldparentpath:
                self.stmts.append(self.get_sql_update_numchild(oldparentpath, "dec"))
            if newparentpath:
                self.stmts.append(self.get_sql_update_numchild(newparentpath, "inc"))
...
class MP_Node(Node):
    ...
    def find_problems(cls):
        ...
        for node in cls.objects.all():
            ...
            real_numchild = (
                ...
                .extra(
                    where=[
                        (sql_length("path", vendor=vendor) + "/%d=%d")
                        % (cls.steplen, node.depth + 1)
                    ]
                )
                .count()
            )
    ...
    def fix_tree(cls, destructive=False, fix_paths=False):
        ...
        if cls.get_database_vendor("read") == "mysql":
            sql = (
                ...
                + sql_concat("tbn1.path", "%%s", vendor=vendor)
                + ") AS real_numchild "
            ...
            subquery = (
                "(SELECT COUNT(1) FROM %(table)s AS tbn2"
                " WHERE tbn2.path LIKE "
                + sql_concat("tbn1.path", "%%s", vendor=vendor)
                + ")"
            )
            ...
            sql = sql % {"table": connection.ops.quote_name(cls._meta.db_table)}
        ...
        sql = (
            "UPDATE %(table)s "
            "SET numchild=%%s "
            "WHERE path=%%s" % {"table": connection.ops.quote_name(cls._meta.db_table)}
        )
    ...
    def get_siblings(self):
        ...
        qset = (
            ...
            .objects.filter(depth=self.depth)
            .order_by("path")
        )
        if self.depth > 1:
    ...
    def _get_path(cls, path, depth, newstep):
        ...
        return "{0}{1}{2}".format(
            parentpath, cls.alphabet[0] * (cls.steplen - len(key)), key
        )
