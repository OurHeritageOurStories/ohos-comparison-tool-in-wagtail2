...
class WillowRegistry(object):
        ...
        if with_operation:
            ...
            if not image_classes:
                raise UnrecognisedOperationError(
                    "Could not find image class with the '{0}' operation".format(
                        with_operation
                    )
                )
        ...
        if available:
            ...
            if not available_image_classes:
                raise UnavailableOperationError(
                    "\n".join(
                        [
                            "The operation '{0}' is available in the following image classes but they all raised errors:".format(
                                with_operation
                            )
                        ]
                        + [
    ...
    def find_all_paths(self, start, end, path=[], seen_classes=set()):
        ...
        if (
            not start in self._registered_image_classes
