...
class TestAccessibilityCheckerConfig(WagtailTestUtils, TestCase):
    ...
    def test_custom_message(self):
        ...
        with hooks.register_temporarily(
            "construct_wagtail_userbar", self.get_hook(CustomMessageAccessibilityItem),
        ):
            config = self.get_config()
            self.assertEqual(
                config["messages"],
    ...
    def test_unset_run_only(self):
        ...
        with hooks.register_temporarily(
            "construct_wagtail_userbar", self.get_hook(UnsetRunOnlyAccessibilityItem),
        ):
            config = self.get_config()
            self.assertEqual(
                config["options"],
    ...
    def test_custom_context(self):
        ...
        with hooks.register_temporarily(
            "construct_wagtail_userbar", self.get_hook(CustomContextAccessibilityItem),
        ):
            config = self.get_config()
            self.assertEqual(
                config["context"],
    ...
    def test_custom_run_only_and_rules_per_request(self):
        ...
        with hooks.register_temporarily(
            "construct_wagtail_userbar", self.get_hook(CustomRunOnlyAccessibilityItem),
        ):
            config = self.get_config()
            self.assertEqual(
                config["options"],
                {"runOnly": CustomRunOnlyAccessibilityItem.axe_run_only, "rules": {},},
            )
