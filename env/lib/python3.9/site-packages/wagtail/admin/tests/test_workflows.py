...
class BasePageWorkflowTests(WagtailTestUtils, TestCase):
    ...
    def setUp(self):
        ...
        self.submitter = self.create_user(
            username="submitter", email="submitter@email.com", password="password",
        )
        editors = Group.objects.get(name="Editors")
        ...
        self.superuser = self.create_superuser(
            username="superuser", email="superuser@email.com", password="password",
        )
...
class BaseSnippetWorkflowTests(BasePageWorkflowTests):
    ...
    def setup_object(self):
        self.object = self.model.objects.create(
            text="Hello world!", live=False, has_unpublished_changes=True,
        )
...
class TestSubmitPageToWorkflow(BasePageWorkflowTests):
    ...
    def test_submit_for_approval_changes_lock_status(self):
        ...
        self.assertNotContains(
            response, f"Only reviewers can edit and approve the {self.model_name}",
        )
        ...
        self.assertNotContains(
            response, f"Anyone can edit this {self.model_name}",
        )
    ...
    def test_workflow_action_menu_items(self):
        ...
        self.assertNotContains(
            response, '<button type="submit" class="button action-save " disabled>',
        )
    ...
    def test_workflow_action_menu_items_when_reverting(self):
        ...
        revert_url = self.get_url(
            "revisions_revert", args=(quote(self.object.pk), old_revision.id),
        )
    ...
    def test_cancel_workflow(self):
        ...
        self.assertNotContains(
            response, f"The {self.model_name} could not be saved as it is locked",
        )
        self.assertNotContains(
            response,
        ...
        self.assertNotContains(
            response, "The snippet could not be saved due to errors",
        )
...
class TestApproveRejectPageWorkflow(BasePageWorkflowTests):
    ...
    def test_workflow_action_view_not_in_moderation(self):
        ...
        # Cancel the workflow
        response = self.client.post(
            self.get_url("edit"), {"action-cancel-workflow": "True"},
        )
    ...
    def test_workflow_report_export(self):
        for export_format in self.export_formats:
            with self.subTest(export_format=export_format):
                response = self.client.get(
                    reverse("wagtailadmin_reports:workflow"), {"export": export_format},
                )
...
class TestPageNotificationPreferences(BasePageWorkflowTests):
    def setUp(self):
        ...
        self.moderator2 = self.create_user(
            username="moderator2", email="moderator2@email.com", password="password",
        )
        moderators = Group.objects.get(name="Moderators")
...
class TestPageWorkflowPreview(BasePageWorkflowTests):
    ...
    def test_preview_workflow_bad_task_id(self):
        preview_url = self.get_url(
            "workflow_preview", args=(quote(self.object.pk), self.task_2.id),
        )
    ...
    def test_preview_workflow_nonexistent_ids(self):
        preview_url = self.get_url(
            "workflow_preview", args=(quote(self.object.pk), 123),
        )
        response = self.client.get(preview_url)
