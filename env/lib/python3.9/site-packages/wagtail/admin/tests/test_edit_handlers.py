...
class TestGetFormForModel(TestCase):
    ...
    def test_tag_widget_is_passed_tag_model(self):
        ...
        form = RestaurantPageForm(
            {"title": "Buonasera", "slug": "buonasera", "tags": "Italian, delicious",}
        )
...
class TestPageEditHandlers(TestCase):
    ...
    def test_repr(self):
        ...
        self.assertIn(
            "model=<class 'wagtail.test.testapp.models.ValidatedPage'>", handler_repr,
        )
...
class TestTabbedInterface(WagtailTestUtils, TestCase):
    def setUp(self):
        ...
        self.event_page_tabbed_interface = TabbedInterface(
            [
                ...
                ObjectList(
                    [InlinePanel("speakers", label="Speakers"),], heading="Speakers",
                ),
                ObjectList(
                    [FieldPanel("cost", permission="superuser"),], heading="Secret",
                ),
                ObjectList(
                    [FieldPanel("cost"),],
    ...
    def test_render(self):
        ...
        tabbed_interface = self.event_page_tabbed_interface.get_bound_panel(
            instance=event, form=form, request=self.request,
        )
    ...
    def test_tabs_permissions(self):
        ...
        with self.subTest("Super user test"):
            ...
            self.assertIn(
                '<a id="tab-label-secret" href="#tab-secret" ', result,
            )
            self.assertIn(
                '<a id="tab-label-custom_setting" href="#tab-custom_setting" ', result,
            )
            self.assertIn(
...
class TestObjectList(TestCase):
    ...
    def test_render(self):
        ...
        self.assertIn(
            '<label for="id_date_from" id="id_date_from-label">', result,
        )

        # result should include help text for children
        self.assertInHTML(
            '<div class="help">Not required if event is on a single day</div>', result,
        )
...
class TestFieldPanel(TestCase):
    ...
    def test_override_heading(self):
        ...
        end_date_panel_with_overridden_heading = end_date_panel_with_overridden_heading.get_bound_panel(
    ...
    def test_render_html(self):
        ...
        field_panel = self.end_date_panel.get_bound_panel(
            instance=self.event, form=form, request=self.request,
        )
    ...
    def test_repr(self):
        form = self.EventPageForm()
        field_panel = self.end_date_panel.get_bound_panel(
            form=form, instance=self.event, request=self.request,
        )
...
class TestFieldRowPanelWithChooser(TestCase):
    def setUp(self):
        ...
        self.dates_panel = FieldRowPanel(
            [FieldPanel("date_from"), FieldPanel("feed_image"),]
        ).bind_to_model(EventPage)
...
class TestPanelIcons(WagtailTestUtils, TestCase):
    ...
    def test_override_inlinepanel_icon(self):
        cases = [
            (InlinePanel("carousel_items", label="Carousey", icon="cogs"), "cogs",),
            (
                MultipleChooserPanel(
                    "related_links",
                    label="Linky",
