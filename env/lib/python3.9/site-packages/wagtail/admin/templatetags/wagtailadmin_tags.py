...
def page_header_buttons(context, page, page_perms):
    ...
    return {
        ...
        "classes": ["w-flex", "w-justify-center", "w-items-center", "w-h-slim-header",],
        "button_classes": [
            "w-p-0",
            "w-w-12",
            "w-h-full",
...
def status(
    label=None, classname=None, url=None, title=None, hidden_label=None, attrs=None,
):
    """
...
def sidebar_props(context):
    ...
    return json_script(
        {"modules": JSContext().pack(modules),}, element_id="wagtail-sidebar-props",
    )
...
def wagtail_config(context):
    ...
    config = {
        ...
        "ADMIN_URLS": {"DISMISSIBLES": reverse("wagtailadmin_dismissibles"),},
    }
