...
class ChooserViewSet(ViewSet):
    ...
    def chosen_view(self):
        return self.chosen_view_class.as_view(model=self.model,)

    @property
    def chosen_multiple_view(self):
        return self.chosen_multiple_view_class.as_view(model=self.model,)
    ...
    def get_block_class(self, name=None, module_path=None):
        ...
        """
        meta = type("Meta", (self.base_block_class._meta_class,), {"icon": self.icon,},)
        cls = type(
            name or "%sChooserBlock" % self.model_name,
            (self.base_block_class,),
            {"target_model": self.model, "widget": self.widget_class(), "Meta": meta,},
        )
        if module_path:
