...
class ImageQuerySet(SearchableQuerySetMixin, models.QuerySet):
    def prefetch_renditions(self, *filters):
        ...
        return self.prefetch_related(
            models.Prefetch(
                "renditions", queryset=queryset, to_attr="prefetched_renditions",
            )
        )
...
class AbstractImage(ImageFileMixin, CollectionMember, index.Indexed, models.Model):
    ...
    search_fields = CollectionMember.search_fields + [
        ...
        index.RelatedFields(
            "tags",
            [index.SearchField("name", boost=10), index.AutocompleteField("name"),],
        ),
    ...
    def generate_rendition_file(self, filter: "Filter") -> File:
        ...
        logger.debug(
            "Generating '%s' rendition for image %d", filter.spec, self.pk,
        )
        ...
        try:
            ...
            logger.debug(
                "Failed to generate '%s' rendition for image %d", filter.spec, self.pk,
            )
            raise
