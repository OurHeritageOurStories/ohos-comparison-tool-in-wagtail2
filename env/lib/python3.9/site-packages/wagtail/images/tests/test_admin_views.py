...
class TestImageIndexView(WagtailTestUtils, TestCase):
    ...
    def test_default_entries_per_page(self):
        for i in range(1, 33):
            self.image = Image.objects.create(
                title="Test image %i" % i, file=get_test_image_file(size=(1, 1)),
            )
    ...
    def test_tags(self):
        image_two_tags = Image.objects.create(
            title="Test image with two tags", file=get_test_image_file(),
        )
    ...
    def test_tag_filtering(self):
        Image.objects.create(
            title="Test image with no tags", file=get_test_image_file(),
        )

        image_one_tag = Image.objects.create(
            title="Test image with one tag", file=get_test_image_file(),
        )
...
class TestImageListingResultsView(WagtailTestUtils, TransactionTestCase):
    ...
    def test_search(self):
        monster = Image.objects.create(
            title="A scary monster", file=get_test_image_file(),
        )
...
class TestImageAddView(WagtailTestUtils, TestCase):
    ...
    def test_add_no_file_selected(self):
        response = self.post({"title": "Test image",})
...
class TestImageEditView(WagtailTestUtils, TestCase):
    ...
    def test_edit(self):
        response = self.post({"title": "Edited",})
    ...
    def test_edit_with_new_image_file(self):
        ...
        response = self.post(
            {"title": "Edited", "file": SimpleUploadedFile("new.png", file_content),}
        )
    ...
    def test_reupload_same_name(self):
        ...
        response = self.post({"title": self.image.title, "file": new_file,})
        self.assertRedirects(response, reverse("wagtailimages:index"))
        self.update_from_db()
    ...
    def test_no_thousand_separators_in_focal_point_editor(self):
        large_image = Image.objects.create(
            title="Test image", file=get_test_image_file(size=(1024, 768)),
        )
        response = self.client.get(
    ...
    def test_unique_together_validation_error(self):
        ...
        response = self.post(
            {"title": "Edited", "collection": evil_plans_collection.id,}
        )
...
class TestImageDeleteView(WagtailTestUtils, TestCase):
    ...
    def test_simple(self):
        ...
        self.assertContains(
            response, f'<form action="{self.delete_url}" method="POST">',
        )
...
class TestImageChooserView(WagtailTestUtils, TestCase):
    ...
    def test_construct_queryset_hook_browse(self):
        ...
        Image.objects.create(
            title="Test image not shown", file=get_test_image_file(),
        )
...
class TestImageChooserChosenMultipleView(WagtailTestUtils, TestCase):
    def setUp(self):
        ...
        self.image2 = Image.objects.create(
            title="Another test image", file=get_test_image_file(),
        )

        self.image3 = Image.objects.create(
            title="Unchosen test image", file=get_test_image_file(),
        )
...
class TestMultipleImageUploader(WagtailTestUtils, TestCase):
    ...
    def test_add_error_max_file_size_escaped(self):
        ...
        data = render_to_string(template_name, context=context, request=request,)
...
class TestMultipleImageUploaderWithCustomImageModel(WagtailTestUtils, TestCase):
    ...
    def setUp(self):
        ...
        # Create an image for running tests on
        self.image = CustomImage.objects.create(
            title="test-image.png", file=get_test_image_file(),
        )
...
class TestMultipleImageUploaderWithCustomRequiredFields(WagtailTestUtils, TestCase):
    ...
    def setUp(self):
        ...
        self.uploaded_image = UploadedImage.objects.create(
            file=get_test_image_file(), uploaded_by_user=self.user,
        )
...
class TestGenerateURLView(WagtailTestUtils, TestCase):
    ...
    def test_get_bad_image(self):
        ...
        # Check JSON
        self.assertJSONEqual(
            response.content.decode(), json.dumps({"error": "Cannot find image.",}),
        )
    ...
    def test_get_bad_filter_spec(self):
        ...
        # Check JSON
        self.assertJSONEqual(
            response.content.decode(), json.dumps({"error": "Invalid filter spec.",}),
        )
