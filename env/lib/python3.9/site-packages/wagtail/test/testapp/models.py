...
class CustomImage(AbstractImage):
    ...
    admin_form_fields = Image.admin_form_fields + ("caption", "fancy_caption",)
...
class StreamPage(Page):
    body = StreamField(
        [
            ...
            ("product", StructBlock([("name", CharBlock()), ("price", CharBlock()),]),),
            ("raw_html", RawHTMLBlock()),
            ("books", StreamBlock([("title", CharBlock()), ("author", CharBlock()),]),),
            ("title_list", ListBlock(CharBlock()),),
        ],
        use_json_field=True,
    )
...
class DefaultRichBlockFieldPage(Page):
    body = StreamField([("rich_text", RichTextBlock()),], use_json_field=True,)
...
class CustomRichBlockFieldPage(Page):
    body = StreamField(
        [("rich_text", RichTextBlock(editor="custom")),], use_json_field=True,
    )
...
class PersonPage(Page):
    ...
    content_panels = Page.content_panels + [
        MultiFieldPanel(
            [FieldPanel("first_name"), FieldPanel("last_name"),], "Person",
        ),
...
class Address(index.Indexed, ClusterableModel, Orderable):
    ...
    tags = ClusterTaggableManager(through="tests.AddressTag", blank=True,)
    person = ParentalKey(
...
class DeadlyStreamPage(Page):
    body = StreamField([("title", DeadlyCharBlock()),], use_json_field=True,)
    content_panels = Page.content_panels + [
        FieldPanel("body"),
    ]
