...
class TestSnippetListViewWithSearchableSnippet(WagtailTestUtils, TransactionTestCase):
    ...
    def get(self, params={}):
        return self.client.get(
            reverse("wagtailsnippets_snippetstests_searchablesnippet:list"), params,
        )
...
class TestSnippetCreateView(WagtailTestUtils, TestCase):
    ...
    def test_create_file_upload_multipart(self):
        ...
        self.assertRedirects(
            response, reverse("wagtailsnippets_snippetstests_fileuploadsnippet:list"),
        )
...
class TestCreateDraftStateSnippet(WagtailTestUtils, TestCase):
    ...
    def post(self, post_data={}):
        return self.client.post(
            reverse("wagtailsnippets_tests_draftstatemodel:add"), post_data,
        )
    ...
    def test_get(self):
        ...
        self.assertNotContains(
            response, '<div class="form-side__panel" data-side-panel="status">',
        )
    ...
    def test_publish(self):
        ...
        try:
            ...
            self.assertEqual(
                snippet.live_revision.content["text"], "Draft-enabled Foo, Published",
            )
    ...
    def test_publish_bad_permissions(self):
        ...
        self.user.user_permissions.add(
            add_permission, edit_permission, admin_permission,
        )
        self.user.save()
        ...
        try:
            ...
            self.assertRedirects(
                response,
                reverse(
                    "wagtailsnippets_tests_draftstatemodel:edit", args=[snippet.pk],
                ),
            )
            ...
            # The revision content should contain the data
            self.assertEqual(
                snippet.latest_revision.content["text"], "Draft-enabled Foo",
            )
    ...
    def test_publish_with_publish_permission(self):
        ...
        self.user.user_permissions.add(
            add_permission, publish_permission, admin_permission,
        )
        self.user.save()
...
class TestEditDraftStateSnippet(BaseTestSnippetEditView):
    ...
    def test_get(self):
        ...
        self.assertNotContains(
            response, f'<a class="button action-secondary" href="{unpublish_url}">',
        )
        self.assertNotContains(response, "Unpublish")
    ...
    def test_publish(self):
        ...
        try:
            ...
            # The revision content should contain the new data
            self.assertEqual(
                latest_revision.content["text"], "Draft-enabled Bar, Published",
            )
    ...
    def test_publish_bad_permissions(self):
        ...
        try:
            ...
            # The revision content should contain the data
            self.assertEqual(
                self.test_snippet.latest_revision.content["text"], "Edited draft Foo",
            )
    ...
    def test_publish_with_publish_permission(self):
        ...
        self.user.user_permissions.add(
            edit_permission, publish_permission, admin_permission,
        )
        self.user.save()
    ...
    def test_save_draft_then_publish(self):
        ...
        # The revision content should contain the new data
        self.assertEqual(
            latest_revision.content["text"], "Draft-enabled Bar, Now Published",
        )
    ...
    def test_publish_twice(self):
        ...
        # The revision content should contain the new data
        self.assertEqual(
            latest_revision.content["text"], "Draft-enabled Bar, Published Twice",
        )
    ...
    def test_edit_post_publish_scheduled_unpublished(self):
        ...
        self.assertNotContains(
            response, '<div class="w-label-3">Once published:</div>', html=True,
        )
        self.assertContains(
            response,
            f'<span class="w-text-text-label">Go-live:</span> {rendered_timestamp(go_live_at)}',
    ...
    def test_edit_post_save_schedule_before_a_scheduled_expire(self):
        ...
        self.assertEqual(
            self.test_snippet.expire_at, expire_at.replace(second=0, microsecond=0),
        )
...
class TestScheduledForPublishLock(BaseTestSnippetEditView):
    ...
    def test_edit_post_scheduled_for_publishing(self):
        ...
        self.assertEqual(
            self.test_snippet.latest_revision.content["text"], "I've been edited!",
        )
...
class TestSnippetDelete(WagtailTestUtils, TestCase):
    ...
    def test_delete_get(self):
        delete_url = reverse(
            "wagtailsnippets_tests_advert:delete", args=[quote(self.test_snippet.pk)],
        )
...
class TestSnippetRevisions(WagtailTestUtils, TestCase):
    ...
    def test_get_with_previewable_snippet(self):
        ...
        # Message should be shown
        self.assertContains(
            response, "You are viewing a previous version of this", count=1,
        )
    ...
    def test_replace_with_limited_permissions(self):
        ...
        response = self.post(
            post_data={"text": "test text", "revision": self.initial_revision.pk,}
        )
        self.assertEqual(response.status_code, 302)
...
class TestSnippetChooseResults(WagtailTestUtils, TestCase):
    ...
    def test_no_results(self):
        ...
        self.assertTemplateUsed(response, "wagtailsnippets/chooser/results.html")
        self.assertContains(
            response, 'href="/admin/snippets/tests/advert/add/"',
        )
...
class TestAddOnlyPermissions(WagtailTestUtils, TestCase):
    ...
    def test_get_edit(self):
        response = self.client.get(
            reverse(
                "wagtailsnippets_tests_advert:edit", args=[quote(self.test_snippet.pk)],
            )
        )
...
class TestSnippetViewWithCustomPrimaryKey(WagtailTestUtils, TestCase):
    ...
    def get(self, snippet, params={}):
        args = [quote(snippet.pk)]
        return self.client.get(
            reverse(snippet.snippet_viewset.get_url_name("edit"), args=args), params,
        )

    def post(self, snippet, post_data={}):
        args = [quote(snippet.pk)]
        return self.client.post(
            reverse(snippet.snippet_viewset.get_url_name("edit"), args=args), post_data,
        )

    def create(self, snippet, post_data={}, model=Advert):
        return self.client.post(
            reverse(snippet.snippet_viewset.get_url_name("add")), post_data,
        )
