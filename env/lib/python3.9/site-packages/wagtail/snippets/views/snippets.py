...
class ModelIndexView(generic.IndexView):
    ...
    def get_columns(self):
        return [
            TitleColumn(
                "name", label=_("Name"), get_url=self.get_list_url, sort_key="name",
            ),
            Column("count", label=_("Instances"), sort_key="count",),
        ]
    ...
    def get_context_data(self, **kwargs):
        ...
        if ordering in ["count", "-count"]:
            snippet_types = sorted(
                self.snippet_types, key=lambda type: type["count"], reverse=reverse,
            )
        else:
            snippet_types = sorted(
                self.snippet_types,
...
class SnippetHistoryReportFilterSet(WagtailFilterSet):
    action = django_filters.ChoiceFilter(
        label=_("Action"), choices=log_registry.get_choices,
    )
...
class SnippetViewSet(ModelViewSet):
    ...
    def preview_on_add_view(self):
        return self.preview_on_add_view_class.as_view(
            model=self.model, form_class=self.get_form_class(),
        )
    ...
    def menu_item_class(self):
        ...
        return type(
            f"{self.model.__name__}MenuItem", (MenuItem,), {"is_shown": is_shown},
        )
